...
#找發文作者
tmls <- get_timelines(c("iingwen"), n = 18000)
...
tmls %>%
  ...
  ggplot2::labs(
    x = NULL, y = NULL,
    title = "Frequency of Twitter statuses posted by iingwen"
  )
...
#取2016後的文章
rt<-tmls %>% dplyr::filter(created_at > "2016-01-01")
...
#去除停詞
stop_en <- c("https", "t.co", "rt", "amp", "trump")
